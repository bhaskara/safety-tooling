...
class LLMResponse(pydantic.BaseModel):
    ...
    def parse_stop_reason(cls, v: str):
        ...
        elif v in ["content_filter", "refusal"]:
            # "refusal" is Anthropic's API-level refusal stop reason (newer
            # models, e.g. Haiku 4.5); map to CONTENT_FILTER so callers treat
            # the response as unusable content instead of crashing (local patch).
